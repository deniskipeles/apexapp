...
# 2. Download ApexKit (aarch64 Apple from Hugging Face)
...
# 3. Download Cloudflared (macOS arm64)
CF_TARGET_FILE="${TARGET_DIR}/cloudflared-${TARGET_TRIPLE}"
if [ ! -f "$CF_TARGET_FILE" ]; then
    echo "📥 Downloading cloudflared (macOS arm64)..."
    curl -L "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-darwin-arm64.tgz" -o "$TEMP_DIR/cloudflared.tgz"
...
# 4. Download FRPC (macOS arm64) - Direct static download (No API rate limits)
FRPC_TARGET_FILE="${TARGET_DIR}/frpc-${TARGET_TRIPLE}"
if [ ! -f "$FRPC_TARGET_FILE" ]; then
    echo "📥 Downloading frpc v${FRP_VER} (macOS arm64)..."
    curl -L "https://github.com/fatedier/frp/releases/download/v${FRP_VER}/frp_${FRP_VER}_darwin_arm64.tar.gz" -o "$TEMP_DIR/frp.tar.gz"
    tar -xzf "$TEMP_DIR/frp.tar.gz" -C "$TEMP_DIR"
    mv "$TEMP_DIR/frp_${FRP_VER}_darwin_arm64/frpc" "$FRPC_TARGET_FILE"
